...
def calculate_centrality(adjacency_matrix, node_idx):

    print(adjacency_matrix)

    # return Centrality
    return np.sum(adjacency_matrix[node_idx])
...
for K in range(Kmin, Kmax + 1):
    ...
    for cluster_idx in range(K):
        ...
        for node_idx in cluster_nodes:
            ace_cluster += np.sum(np.square(pairwise_distances(adjacency_matrix[node_idx].reshape(1, -1),
                                                               adjacency_matrix[estimated_center].reshape(1, -1))))
